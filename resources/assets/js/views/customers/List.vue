    ...
    <v-container fluid pa-0>
        <v-card>
            <v-card-title primary-title class="primary white--text pt-3 pb-2">
                <h2 class="headline" py-0 my-0>Clientes</h2>
            </v-card-title>
            <v-card-text>
                <v-layout row wrap>
                ...
                <v-layout row wrap>
                    ...
                    <v-flex xs12 md4 lg4 class="text-xs-right">
                        ...
                        >   
                            <v-icon>add</v-icon> Criar Cliente
                        </v-btn>
                    </v-flex>
                </v-layout>
            </v-card-text>
                ...
                <template slot="items" slot-scope="props">
                    <tr>
                        ...
                        <td class="text-xs-center px-0">
                            <v-tooltip bottom>
                                <v-btn class="mx-0" slot="activator" flat icon color="warning" :to="{ name: 'customers.edit', params: { id: props.item.id }}">
                                    <v-icon>edit</v-icon>
                                </v-btn>
            ...
            >
                ...
                <template slot="items" slot-scope="props">
                    <tr>
                        ...
                        <td class="text-xs-center px-0">
                            <v-tooltip bottom>
                                <v-btn class="mx-0" slot="activator" flat icon color="info" :to="{ name: 'customers.edit', params: { id: props.item.id }}">
                                    <v-icon>edit</v-icon>
                                </v-btn>
                                <span>Editar Cliente</span>
                            </v-tooltip>
...
<script>
    export default {
        ...
        methods : {
            ...
            deactivate(id) {
                axios
                .get('/api/customers/'+id+'/deactivate')
                .then(response => {
                    if(response.data.success){
            ...
            reactivate(id) {
                axios
                .get('/api/customers/'+id+'/reactivate')
                .then(response => {
                    if(response.data.success){
                        this.employees.forEach(e => {
                            if(e.id == id) {
