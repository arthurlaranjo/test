<template>
    <v-container fluid pa-0>
        <v-card>
            <v-card-title primary-title class="primary white--text pt-3 pb-2"><h2 class="headline">Fornecedores</h2></v-card-title>
            <v-card-text>
                <v-layout row wrap>
                ...
                <template slot="items" slot-scope="props">
                    <tr>
                        ...
                        <td class="text-xs-center px-0">
                            <v-tooltip bottom>
                                <v-btn class="mx-0" slot="activator" flat icon color="warning" :to="{ name: 'suppliers.edit', params: { id: props.item.id }}">
                                    <v-icon>edit</v-icon>
                                </v-btn>
...
<script>
    export default {
        data () {
            return {
                ...
                headers: [
                    ...
                    { text: 'Nome Fantasia', align: 'left', value: 'trade_name' },
                    { text: 'Telefone', align: 'left', value: 'phone' },
        ...
        methods : {
            ...
            deactivate(id) {
                axios
                .get('/api/suppliers/'+id+'/deactivate')
                .then(response => {
            ...
            reactivate(id) {
                axios
                .get('/api/suppliers/'+id+'/reactivate')
                .then(response => {
                    if(response.data.success){
                        this.suppliers.forEach(e => {
                            if(e.id == id) {
